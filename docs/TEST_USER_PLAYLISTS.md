...
1. 此接口仍依赖有效的 QQ 音乐 Cookie
2. 上游接口字段结构可能变化，项目当前会做标准化，只对外返回 `playlists`
3. 当上游仅支持分页请求时，项目会在服务端尽量兼容 `offset`，但结果仍受上游分页能力限制

## 可用的替代接口

根据网络搜索，以下 QQ 音乐 API 接口目前仍可使用：
...
2. **已知问题**: QQ 音乐 API 接口可能已更新，如果返回 404 错误，说明该接口暂时不可用
